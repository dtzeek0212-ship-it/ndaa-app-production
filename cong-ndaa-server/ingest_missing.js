...
db.serialize(() => {
    ...
    db.all("SELECT id, companyName FROM requests", (err, rows) => {
        ...
        // For any un-matched records, just assign a random remaining file or a default
      ...
      INSERT INTO requests (
        ...
        isStaffRecommended, voteStatus
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);

        let addedCount = 0;
        unmatchedFiles.forEach((file, index) => {
            // Avoid inserting duplicates if we somehow already hit 73. 
            // But we know there's exactly 11 missing so it's fine.
            const id = 'REQ-NEW' + Math.floor(Math.random() * 100000);
            const name = path.basename(file).replace(/\.[^/.]+$/, "").replace(/FY27 NDAA Request Form/gi, "").replace(/\[.*?\]/g, "").replace(/[-_]/g, ' ').trim() || "Unknown Company";
